-- Evolvee Radiance Operations Hub — PostgreSQL schema (v2 scope)
...
-- Points of contact at each manufacturer (a manufacturer can have several).
...
-- Product/SKU registry. Mirrors SKUs from Zoho Inventory and links each
-- SKU to the manufacturer responsible for producing it.
...
-- Reorder threshold per SKU. When live stock drops to/below the threshold,
-- the cron job writes a reorder alert.
...
-- Free-text communication log against a manufacturer record.
...
-- Basic status tracking for active production runs / outstanding orders.
...
-- Records the outcome of every external data sync so failures are surfaced
-- to the user instead of silently showing stale data (spec: Reliability).
