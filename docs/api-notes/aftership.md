...
**into the backend** (inbound). The Hub also makes outbound calls to create/update
trackings and read courier data. Keep this directionality in mind: most shipping data
arrives *unsolicited* at a webhook endpoint, not via polling.
...
Store the key in `.env` as `AFTERSHIP_API_KEY`. AfterShip also offers a more involved
HMAC/RSA-signed key method and OAuth (30-day access tokens) for distributed apps; the
plain API key is sufficient for an internal integration.

---

## Outbound endpoints (Hub → AfterShip)
...
## Webhooks (AfterShip → Hub) — the primary data path
...
the payload. Store the secret in `.env` as `AFTERSHIP_WEBHOOK_SECRET`.
...
(previously ~10 requests/sec per organization) to **per-endpoint** rate limits. Each
