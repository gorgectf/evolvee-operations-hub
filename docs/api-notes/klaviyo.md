...
**Integration role:** **Outbound only.** The Hub *sends* events and profile data to
Klaviyo; nothing is pulled back in V1/V2 scope. Marketing Performance analytics (reading
Klaviyo data back out) is deferred to V2.
...
Store the chosen revision as a constant in code (e.g. `KLAVIYO_API_REVISION`) so every
request sends the same one.
...
|---|---|---|
| **Private API key** | Server-side `/api` endpoints (what the Hub uses) | `Authorization: Klaviyo-API-Key {private_key}` |
...
> and writes sensitive account data — treat it like a password, keep it in `.env`
> (`KLAVIYO_PRIVATE_KEY`), and scope it to only the permissions this integration needs.
> A new private key can be created/deactivated at any time if exposed.
...
## Outbound endpoints (Hub → Klaviyo)
