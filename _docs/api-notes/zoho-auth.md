...
*Authentication reference for Zoho CRM*

The OAuth flow below is what the CRM integration uses. `zoho-crm.md` lists the CRM
endpoints and scope and points back here for auth.
...
The Zoho web app URL tells you the region (e.g. `crm.zoho.eu` → EU). Set both in your
...
2. **Generate a grant token** for the CRM scope:
...
4. **Access tokens expire after ~1 hour.** The **refresh token does not expire** unless
   revoked - store it and exchange it for fresh access tokens as needed. The backend
   caches the access token and refreshes it automatically.
...
- Zoho enforces **per-minute request limits** and a **per-day API credit** model that
  varies by plan/edition.
- **No `Retry-After` header** - implement your own throttle/back-off (e.g. start at 1s,
  double on repeat 429) rather than reading a server-provided delay.
- **Retry on `429`.**
