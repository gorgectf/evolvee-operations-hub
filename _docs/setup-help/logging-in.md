...
The seed creates one account per role. **All seed passwords are `radiance123`** — change
them before any real use (Admin → Team Members page, or re-seed with new values in
`backend/db/seed.js`).
